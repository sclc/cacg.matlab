function [X,hist_residual] = bcbcg_chapman_saad_1_qr(A, RHS, s_k, blocksize, X, eigenValue_max, eigenValue_min,maxIters, tol)
...
% [eigenVecCluster,eigenValCluster] = eigs (A, blocksize-1, 'lm');
[eigenVecCluster,eigenValCluster] = eigs (A, blocksize-1, 'sm');
% [eigenVecCluster,eigenValCluster] = eigs (A, blocksize, 'lm');

s_alpha = 2.0 / (eigenValue_max - eigenValue_min);
s_beta  = - (eigenValue_max + eigenValue_min) / (eigenValue_max - eigenValue_min);
RHS(:,2:blocksize) = eigenVecCluster(:,1:(blocksize-1));
