function [X,hist_residual] = cacg_monomial_2_1(A, RHS, s_k, blocksize, X, maxIters, tol)
%% block chebyshev basis conjugat gradient
%% basic version
%% inherited from cacg_monomial_2 2014-12-05 version
%% hist_residual saves the R calculated in algorithm
%% test
    ...
    for ind = 1:maxIters
        ...
        for dimInd = 1:s_k
            ...
            elseif dimInd == 2
                ...
                SS(:, blocksize*(dimInd-1)+1:blocksize*dimInd) = A_shift * SS(:, blocksize*(dimInd -2)+1:blocksize*(dimInd -1));
            end           
        end
        % A-orthogonalization 
        if ind == 1
